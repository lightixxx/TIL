...
충돌은 허브에서 발생하기 때문에 스위치 안에서 **수신한 프레임을 따로따로 보낼 수 있도록 처리해서 충돌을 막는다**. 그 방법으로는 **MAC 주소 필터링과 버퍼링**, 두 가지를 시행한다.

먼저, MAC 주소 필터링은 '학습'과 '스위칭'이라는 동작으로 이루어져 있다. 우선 학습인데, 이것은 **수신한 프레임의 송신처 MAC 주소를 기록**하는 것이다. 이것으로 **수신한 포트랑 MAC 주소를 연관 짓는 것**이다. 다음의 스위칭을 위해서 전달되어 온 프레임의 송신처 MAC 주소하고 그것을 수신한 포트의 대응표를 만든다고 볼 수 있다. 이 '학습'에 의해 **스위치는 포트에 연결되어 있는 컴퓨터의 MAC 주소를 기억**하는 것이다. 즉, '이 포트에 이 MAC 주소를 가진 컴퓨터가 있습니다.'라고 기억하는 것이다. 이 대응표를 **어드레스 테이블**이라고 한다.

프레임을 수신한 스위치는 **프레임의 수신처 MAC 주소를 보고 그 MAC 주소가 있는 포트만 프레임을 송신**하는 것이다. 그 외의 다른 포트는 송신하지 않는 것이다. 그럼 여기에서 아까 '학습'한 어드레스 테이블에서 찾아 '수신처 MAC 주소가 있는 포트'를 알 수 있다. 그 결과 **수신처가 다른 프레임이 동시에 스위치에 도달해도 충돌은 발생하지 않게 되는 것**이다. 만약 허브라면 어떤 프레임이 도달해도 플러딩하려고 하니까 동시에 도달한 경우에는 충돌이 발생하지만 스위치에서는 그런 일이 일어나지 않는다는 것이다. 예를 들어 A에서 B로, 그리고 C에서 D로 스위치에 도달했다고 해도 B수신은 B가 연결되어 있는 포트만, D수신은 D가 연결되어 있는 포트만 송신하기 때문에 충돌하지 않는 것이다.

이 동작은 MAC 주소에 의해 송신하는 포트를 필터링하기 때문에 **MAC 주소 필터링**이라고 한다. 이 MAC 주소 필터링에서 주의할 점은 스위치가 먼저 주소를 '학습'하고 포트와 MAC 주소의 대응표인 어드레스 테이블을 만들어야 한다는 점이다. 만약 그 MAC 주소가 송신처의 프레임을 수신하기 전이어서 '학습하지 않은' 상태라면 MAC 주소 필터링을 할 수 없기 때문이다. '학습 전의 MAC 주소 수신의 프레임을 수신한' 경우랑 수신처가 1대가 아닌 멀티캐스트와 브로드캐스트 수신처인 프레임을 수신한 경우에는 허브와 마찬가지로 플러딩을 하게 되는 것이다.
...
수신처가 같은 프레임이 동시에 도달했을 때 버퍼링이 제 역할을 한다. 버퍼링은 **버퍼(buffer)** 를 사용한 처리를 실행하는 것을 말하는데, 버퍼는 일시적으로 데이터를 기록해 둘 수 있는 기억기기(메모리)이다. 수신처가 같은 프레임이 동시에 도달했을 때를 생각해보면, 그 수신처가 연결되어 있는 포트로부터 송신을 하려고 할텐데 한 개밖에 없는 출구에 프레임이 두 개가 있어서 충돌이 발생한다. 그래서 충돌이 일어나지 않도록 **충돌할 것 같은 프레임을 버퍼에 일시적으로 저장**해 둔다. 수신처가 같은 프레임이 두 개 도달했을 경우 한 개는 송신하고 나머지 한 개는 일시적으로 버퍼에 저장하는 것이다. 그리고 첫 번째 프레임 송신이 끝나면 저장해 두었던 프레임을 송신시키는 것이 **버퍼링**이다.

다만 여기에서 문제가 되는 것은 **버퍼의 용량**이다. 만약 수신처가 같은 프레임이 계속해서 올 경우 메모리의 용량은 제한되어 있기 때문에 버퍼에 저장할 수는 있지만 무한대로 저장할 수는 없기 때문이다. 이 경우에는 **백 프레셔(Back Pressure)** 또는 **IEEE802.3x**라는 규격을 사용해서 송신을 조정한다. 간단히 말하면 버퍼가 부족할 것 같다고 판단하면 송신을 중지하는 것이다. 2종류가 있는 이유는 스위치가 전이중 이더넷에 대응하면 IEEE802.3x를 사용하고 그렇지 않으면 백프레셔 방식을 사용하기 때문이다. 즉, MAC 주소 필터링과 버퍼링을 사용해서 충돌이 발생하지 않도록 하는 것이다.

충돌 도메인을 떠올려보면, 충돌 도메인은 '그 번위에 있는 기기에서 충돌 가능성이 있다'는 범위를 말하는 것인데, 스위치를 사용하면 충돌이 발생하지 않게 되는 것이다. 즉, **충돌 도메인은 스위치에 의해 분할**되는 것이다. 충돌 도메인은 작아야 하는데 스위치가 이것을 실현해서 데이터 통신의 효율을 높이는 것이다.

### 전이중 이더넷

'스위치를 사용하면 충돌이 발생하지 않게 된다.'고 했는데, 여기서 기억해야 할 것은 이더넷에서는 되도록 충돌을 방지하기 위해 CSMA/CD를 사용한다는 것이다. 그런데 스위치를 사용하면 충돌이 일어나지 않는데 굳이 CSMA/CD까지 사용해 충돌을 막으려고 하는 것은 쓸모없는 일이라는 생각이 들 수 있다. 게다가 **CSMA/CD는 반이중 통신**이라 효율이 나쁜 구조이다. 반이중 통신(Half-Duplex)이란 '누군가가 송신 중(자기는 수신 중)일 때는 송신 불가능', '자기가 송신 중일 때는 수신 불가능'같은 통신 방식을 말하는 것이다. 트랜시버와 마찬가지로 자기가 말하면 다른 사람 소리는 안 들리고, 반대로 듣고 있을 때는 이쪽에서 말할 수 없는 것과 같은 개념이다. 일단 신호 감지가 있어서 '누군가가 송신 중일 때는 데이터를 송신하지 않는다', 그리고 '자신이 송신 중일 때는 충돌을 막기 위해 다른 컴퓨터가 송신해 오지 않는다'. 즉, 수신하지 않는다는 것이다. 그래서 CSMA/CD는 반이중 통신이라는 것이다.

반면 '동시에 송신과 수신을 할 수 있는' 방식은 **전이중 통신(Full-Duplex)** 이라고 부른다. 스위치를 사용한 경우엔 충돌을 염려할 필요가 없으니까 CSMA/CD를 사용할 필요가 없다. 그래서 **전이중 통신을 할 수 있다**는 것이다. 이렇게 스위치를 사용해 전이중 통신을 하는 것을 **전이중 이더넷**이라고 한다. 예를 들어 5대의 컴퓨터가 허브에 연결되어 있고 케이블이 100Mbps라고 치면, 5대의 컴퓨터가 각각 100Mbps의 데이터를 송신하고 싶은 경우 허브에서는 동시에 송신을 할 수 없기 때문에 1대가 100Mbps를 송신한 후에 다음 컴퓨터가 보내고, 또 다음으로.. 이런식으로 진행하므로 결과적으로 5초가 걸린다. 1대당 20Mbps분 밖에 사용할 수 없다는 계산이 되는 것이다.

하지만 스위치를 사용한 전이중 이더넷이라면 동시에 송신과 수신이 가능하기 때문에 100Mbps를 송신하는데 걸리는 시간은 1초, 컴퓨터 1대 당 그대로 100Mbps를 사용할 수 있는 것이다. CSMA/CD로 허브에 100대 연결되어 있으면 1/100의 1Mbps가 되는 것이다. 다시말해 접속 대수로 나눈 것 만큼 사용할 수 있는데, 전이중 이더넷이라면 케이블 부분을 그대로 사용할 수 있다. 그래서 현재는 스위치가 보급되어서 전이중 이더넷이 당연하게 된 것이다.

> - 버퍼링으로 수신처가 같은 플레임의 충돌을 막는다.
